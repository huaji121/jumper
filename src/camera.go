...
type Camera struct {
	...
	FixedX      float64 // world position for fixed mode
	FixedY      float64
}
...
// SetFixed locks the camera at a world position.
...
// Update moves the camera toward its target (follow mode) or stays at the
// fixed position, then clamps to map bounds.
func (c *Camera) Update(mapPW, mapPH int) {
	if c.Mode == "fixed" {
		c.X += (c.FixedX - c.X) * c.FollowSpeed
		c.Y += (c.FixedY - c.Y) * c.FollowSpeed
	} else {
